<script>
    import { formatDateFR } from "$lib/globalFunction";
    export let movie;
...
>
    ...
    <div class="flex flex-col px-4 py-3">
        <span class="font-semibold text-white truncate text-sm">{movie.title}</span>
        <span class="text-gray-400 truncate text-xs">{formatDateFR(movie.release_date)}</span>
    </div>
</a>